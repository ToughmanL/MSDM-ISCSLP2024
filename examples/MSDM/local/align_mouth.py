...
if __name__ == '__main__':
    ...
    fids = read_datalist(args.filename_path)
    ...
    for filename_idx, fid in enumerate(tqdm(fids)):
        filename = fid['key']
        path = fid['wav'].split('.')[0]

        video_pathname = path + '.avi'
        landmarks_pathname = os.path.join(args.landmark_direc, filename+'.pkl')

        dst_pathname = os.path.join(args.save_direc, filename+'.avi')

        if not os.path.isfile(video_pathname):
            print("File does not exist. Path input: {}".format(video_pathname))
            continue
        ...
        if sequence is None:
            print("cannot crop from {}.".format(filename))
            continue
...
# python align_mouth.py --landmark data/landmarkdata --filename-path data/seg_data/data.list --save-direc data/crop_video --mean-face data/words_mean_face_20.npy --ffmpeg /mnt/shareEEx/liuxiaokang/miniconda3/envs/AVSR/bin/ffmpeg --rank $i --nshard 10